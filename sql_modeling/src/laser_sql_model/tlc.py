# Import a model
import random
import laser_sql_model.sir_sql as model
...
def run_simulation(ctx, csvwriter, num_timesteps):
    ...
    for timestep in range(1, num_timesteps + 1):
        ...
        if sum(counts["I"].values()) == 0:
            ...
            import_node=random.randint(0,settings.num_nodes)
            #model.inject_cases( ctx, import_cases=100, import_node=settings.num_nodes-1 )
            model.inject_cases( ctx, import_node=import_node )
...
if __name__ == "__main__":
    ...
    ctx = model.initialize_database( from_file=False )
    #ctx = model.init_db_from_csv( settings )
    ctx = model.eula_init( ctx, settings.eula_age, eula_strategy="separate" )
